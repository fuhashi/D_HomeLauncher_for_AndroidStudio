...
import jp.co.disney.apps.managed.kisekaeapp.launcher.AppsCustomizePagedView.ContentType;
...
public class AppsCustomizeTabHost extends TabHost implements LauncherTransitionable,
        TabHost.OnTabChangeListener, DrawerFilterView.OnChangedListener {
    static final String LOG_TAG = "AppsCustomizeTabHost";

    private static final String APPS_TAB_TAG = "APPS";
    private static final String WIDGETS_TAB_TAG = "WIDGETS";
    private static final String DISNEY_TAB_TAG = "DISNEY";
    ...
    private DrawerFilterView mDrawerFilterView;
    ...
    protected void onFinishInflate() {
        ...
        mBgImageView = (ImageView) findViewById(R.id.drawer_bg);

        mDrawerFilterView = (DrawerFilterView) findViewById(R.id.apps_filter);
        mDrawerFilterView.setOnChangedListener(this);

        mAppsCustomizePane.setPageIndicator((PageIndicatorView) findViewById(R.id.page_indicator));
        ...
        AccessibleTabView tabView;
        // -- アプリ
        tabView = (AccessibleTabView) mLayoutInflater.inflate(R.layout.tab_widget_indicator, tabs, false);
        tabView.setTextDrawable ((BitmapDrawable) ContextCompat.getDrawable(getContext(), R.drawable.tab_app));
        tabView.setBackgroundResource(R.drawable.drawer_tab_left);
        addTab(newTabSpec(APPS_TAB_TAG).setIndicator(tabView).setContent(contentFactory));
        // -- Disney
        tabView = (AccessibleTabView) mLayoutInflater.inflate(R.layout.tab_widget_indicator, tabs, false);
        tabView.setTextDrawable ((BitmapDrawable) ContextCompat.getDrawable(getContext(), R.drawable.tab_disney));
        tabView.setBackgroundResource(R.drawable.drawer_tab_center);
        tabView.setTextOffsetY((int) Math.ceil(1 * density));
        addTab(newTabSpec(DISNEY_TAB_TAG).setIndicator(tabView).setContent(contentFactory));
        // -- ウィジェット
        tabView = (AccessibleTabView) mLayoutInflater.inflate(R.layout.tab_widget_indicator, tabs, false);
        tabView.setTextDrawable ((BitmapDrawable) ContextCompat.getDrawable(getContext(), R.drawable.tab_widget));
        tabView.setBackgroundResource(R.drawable.drawer_tab_right);
        tabView.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {

                // Android 4.0では「ウィジェット」タブをタップ時にピッカーを表示する
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        if (v.isPressed()) {
                            mLauncher.startAppWidgetPick(false, null);
                            v.setPressed(false);
                        }
                        break;
                    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mBgRawWidth != 0) {
            float scale = MeasureSpec.getSize(widthMeasureSpec) / (float) mBgRawWidth;
            setThemeBackgroundScaleAndPos(scale, -mStatusBarHeight);
        }
    ...
    private void onTabChangedEnd(AppsCustomizePagedView.ContentType type) {
        ...
        if (type == ContentType.Applications) {
            mDrawerFilterView.setVisibility(VISIBLE);
        } else {
            mDrawerFilterView.setVisibility(INVISIBLE);
        }
    ...
    public void onTabChanged(String tabId) {
        ...
        post(new Runnable() {
            ...
            public void run() {
                ...
                ObjectAnimator outAnim = ViewAnimUtils.ofFloat(mAnimationBuffer, "alpha", 0f);
                ...
                ObjectAnimator inAnim = ViewAnimUtils.ofFloat(mAppsCustomizePane, "alpha", 1f);
                ...
                final AnimatorSet animSet = ViewAnimUtils.createAnimatorSet();
    ...
    public void onFilterChanged(int filterCategory, int filterAlphabet, int filterKana) {
        if (mAppsCustomizePane == null) return;
        mAppsCustomizePane.updateFilterState(filterCategory, filterAlphabet, filterKana);
    }
    ...
    public AppsCustomizePagedView.ContentType getContentTypeForTabTag(String tag) {
        ...
        } else if (tag.equals(DISNEY_TAB_TAG)) {
            return AppsCustomizePagedView.ContentType.Disney;
        }
        return AppsCustomizePagedView.ContentType.Applications;
    }
    ...
    public String getTabTagForContentType(AppsCustomizePagedView.ContentType type) {
        ...
        } else if (type == AppsCustomizePagedView.ContentType.Disney) {
            return DISNEY_TAB_TAG;
        }
        return APPS_TAB_TAG;
    }
    ...
    void reset() {
        if (mInTransition) {
            ...
            // Reset immediately
            mDrawerFilterView.reset();
            mDrawerFilterView.invalidate();
    ...
    private FastBitmapDrawable mBg;
    private int mBgRawWidth;

    public void setThemeBackground(Bitmap bg) {
        if (bg == null) return;

        mBg = new FastBitmapDrawable(bg);
        mBgRawWidth = bg.getWidth();

        float scale = getMeasuredWidth() / (float) mBgRawWidth;
        setThemeBackgroundScaleAndPos(scale, -mStatusBarHeight);

        mBgImageView.setImageDrawable(mBg);
    }

    private void setThemeBackgroundScaleAndPos(float scale, int marginTop) {
        mMatrix.reset();
        mMatrix.postScale(scale, scale);
        mMatrix.postTranslate(0, marginTop);
