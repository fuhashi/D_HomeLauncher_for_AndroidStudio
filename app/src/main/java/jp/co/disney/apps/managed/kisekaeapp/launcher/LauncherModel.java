...
import jp.co.disney.apps.managed.kisekaeapp.launcher.InstallWidgetReceiver.WidgetMimeTypeHandlerData;
 ...
 * for the Launcher.
 */
public class LauncherModel extends AbstractContentsLoader {
    static final String TAG = "LauncherModel";

    static final String SP_KEY_INIT_FLG = "INIT_FLG";
    public static final String SP_KEY_KISEKAE_INIT_FLG = "KISEKAE_INIT_FLG";
    public static final String SP_KEY_OS_VERSION = "OS_VERSION";
    ...
    static final ArrayList<DummyInfo> sBgDummyItems = new ArrayList<DummyInfo>();
    ...
    public interface Callbacks extends AbstractCallbacks {
        ...
        public void bindDummyItem(DummyInfo dummyInfo);
    ...
    ArrayList<Runnable> getDeferredBindRunnables() {
        return mDeferredBindRunnables;
    }
    ...
    LauncherModel(LauncherApplication app, IconCache iconCache) {
        ...
        mReceiver = new LauncherReceiver(config.mcc);
    }

    public LauncherReceiver getReceiver() {
        return mReceiver;
    }
    ...
    void unbindWorkspaceItemsOnMainThread() {
        ...
        final ArrayList<DummyInfo> tmpDummyItems = new ArrayList<DummyInfo>();
        synchronized (sBgLock) {
            tmpWorkspaceItems.addAll(sBgWorkspaceItems);
            tmpAppWidgets.addAll(sBgAppWidgets);
            tmpDummyItems.addAll(sBgDummyItems);
        }
        Runnable r = new Runnable() {
                ...
                public void run() {
                   ...
                   for (ItemInfo item : tmpDummyItems) {
                       item.unbind();
                   }
                }
            };
    ...
    protected void showToast(final Context context, final int resId) {
    ...
    ResolveInfo findResolveInfo(Context context, String packageName, String className) {
    ...
    static void moveItemInDatabase(Context context, final ItemInfo item, final long container,
        ...
        DebugLog.instance.outputLog(TAG, transaction);
    ...
    static void getScreenInfoFromDatabase(Context context, int[] numPages, int[] defaultPage) {
        ...
        final Cursor c_head = cr.query(LauncherSettings.Favorites_Head.CONTENT_URI, null, null, null, null);
        try {
            final int numPagesIndex = c_head.getColumnIndexOrThrow(LauncherSettings.Favorites_Head.NUM_PAGES);
            final int defaultPageIndex = c_head.getColumnIndexOrThrow(LauncherSettings.Favorites_Head.DEFAULT_PAGE);

            while (c_head.moveToNext()) {
                numPages[0] = c_head.getInt(numPagesIndex);
                defaultPage[0] = c_head.getInt(defaultPageIndex);
            }
        } finally {
            c_head.close();
        }
    ...
    static String[] getWallpaperPathsFromDatabase(Context context) {

        String[] wpPaths = new String[9];

        final ContentResolver cr = context.getContentResolver();

        final Cursor c_head = cr.query(LauncherSettings.Favorites_Head.CONTENT_URI, null, null, null, null);
        try {
            final int wp1Index = c_head.getColumnIndexOrThrow(LauncherSettings.Favorites_Head.WP_1);
            final int wp2Index = c_head.getColumnIndexOrThrow(LauncherSettings.Favorites_Head.WP_2);
            final int wp3Index = c_head.getColumnIndexOrThrow(LauncherSettings.Favorites_Head.WP_3);
            final int wp4Index = c_head.getColumnIndexOrThrow(LauncherSettings.Favorites_Head.WP_4);
            final int wp5Index = c_head.getColumnIndexOrThrow(LauncherSettings.Favorites_Head.WP_5);
            final int wp6Index = c_head.getColumnIndexOrThrow(LauncherSettings.Favorites_Head.WP_6);
            final int wp7Index = c_head.getColumnIndexOrThrow(LauncherSettings.Favorites_Head.WP_7);
            final int wp8Index = c_head.getColumnIndexOrThrow(LauncherSettings.Favorites_Head.WP_8);
            final int wp9Index = c_head.getColumnIndexOrThrow(LauncherSettings.Favorites_Head.WP_9);

            while (c_head.moveToNext()) {
                wpPaths[0] = c_head.getString(wp1Index);
                wpPaths[1] = c_head.getString(wp2Index);
                wpPaths[2] = c_head.getString(wp3Index);
                wpPaths[3] = c_head.getString(wp4Index);
                wpPaths[4] = c_head.getString(wp5Index);
                wpPaths[5] = c_head.getString(wp6Index);
                wpPaths[6] = c_head.getString(wp7Index);
                wpPaths[7] = c_head.getString(wp8Index);
                wpPaths[8] = c_head.getString(wp9Index);
            }
                ...
                synchronized (sBgLock) {
                    ...
                    switch (item.itemType) {
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_DUMMY:
                            sBgDummyItems.add((DummyInfo) item);
                            break;
                    }
                }
            }
    ...
    static void deleteWidgetsFromDatabase(Context context) {
        final ContentResolver cr = context.getContentResolver();
        final Uri uri = LauncherSettings.Favorites.CONTENT_URI_NO_NOTIFICATION;

        Runnable r = new Runnable() {
            public void run() {
                cr.delete(uri, LauncherSettings.Favorites.ITEM_TYPE + "="
                        + LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET, null);
            }
        };
        runOnWorkerThread(r);
    }
                ...
                synchronized (sBgLock) {
                    switch (item.itemType) {
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET:
                            ...
                            break;
                        case LauncherSettings.Favorites.ITEM_TYPE_DUMMY:
                            sBgDummyItems.remove((DummyInfo) item);
                    }
    ...
    public class LauncherReceiver extends BroadcastReceiver {
        ...
        public LauncherReceiver(int configMcc) {
            mPreviousConfigMcc = configMcc;
        }
            ...
            if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
                ...
                forceReload();
                 ...
                 if (mPreviousConfigMcc != currentConfig.mcc) {
                     DebugLog.instance.outputLog(TAG, "Reload apps on config change. curr_mcc:"
    ...
    protected boolean stopLoaderLocked() {
        ...
        LoaderTask oldTask = (LoaderTask) mLoaderTask;
        if (oldTask != null) {
    ...
    public void startLoader(boolean isLaunching, int synchronousBindPage) {
        startLoader(isLaunching, synchronousBindPage, null, false);
    }
    ...
    public void stopLoader() {
        synchronized (mLock) {
            if (mLoaderTask != null) {
                ((LoaderTask) mLoaderTask).stopLocked();
            }
        }
    }
    ...
    boolean isLoadingWorkspace() {
        synchronized (mLock) {
            if (mLoaderTask != null) {
                return ((LoaderTask) mLoaderTask).isLoadingWorkspace();
            }
        }
        return false;
    }
    ...
    void enqueuePackageUpdated(PackageUpdatedTask task) {
        ...
        // この段階では、まだインストールされたウィジェットの情報が取得できないケースが存在するため、
        // 時間をおいて、再度取得・表示処理を実行する。
        sWorker.postDelayed(new Runnable() {
            @Override
            public void run() {
                bindWidgetsAndShortcus();
            }
        }, 10000);
    }
    ...
    public static ArrayList<AppWidgetProviderInfo> findWidgetByPackageName(Context context, String packageName) {

        ArrayList<AppWidgetProviderInfo> foundList = new ArrayList<AppWidgetProviderInfo>();
    ...
    public static AppWidgetProviderInfo findWidgetByProviderClassName(Context context, String className) {
        ...
        for (AppWidgetProviderInfo provider : providers) {
            if (className.equals(provider.provider.getClassName())) {
                return provider;
            }
        }
    ...
    public ShortcutInfo getShortcutInfo(PackageManager manager, Intent intent, Context context,
        ...
        try {
            ...
            DebugLog.instance.outputLog(TAG, "getPackInfo failed for package " + componentName.getPackageName());
        }
    ...
    Bitmap getIconFromCursor(Cursor c, int iconIndex, Context context) {
        ...
        DebugLog.instance.outputLog(TAG, "getIconFromCursor app="
    ...
    ShortcutInfo infoFromShortcutIntent(Context context, Intent data, Bitmap fallbackIcon) {
        ...
        String shortcutName = data.getStringExtra(InstallShortcutReceiver.EXTRA_SHORTCUT_NAME_USER);
        ...
        if (bitmap != null && bitmap instanceof Bitmap) {
            final int iconSize = context.getResources().getDimensionPixelSize(R.dimen.app_icon_size);
            Bitmap iconBmp = (Bitmap) bitmap;
            if (iconBmp.getWidth() < iconSize) {
                final Matrix matrix = new Matrix();
                iconBmp = BitmapUtils.resizeBitmap2(iconBmp, iconSize, iconSize, matrix, true);
            }
            icon = Utilities.createIconBitmap(new FastBitmapDrawable(iconBmp), context);
            customIcon = true;
        } else {
        ...
        info.shortcutName = shortcutName;
    ...
    void updateSavedIcon(Context context, ShortcutInfo info, byte[] data) {
        ...
        if (needSave) {
            DebugLog.instance.outputLog(TAG, "going to save icon bitmap for info=" + info);
     ...
     * or make a new one.
     */
    static FolderInfo findOrMakeFolder(HashMap<Long, FolderInfo> folders, long id) {
            ...
            if (mLabelCache.containsKey(a)) {
                ...
                if (a instanceof AppWidgetProviderInfo) {
                    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                        ...
                        labelA = ((AppWidgetProviderInfo) a).loadLabel(mPackageManager);
                    }
                } else {
            ...
            if (mLabelCache.containsKey(b)) {
                ...
                if (b instanceof AppWidgetProviderInfo) {
                    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                        ...
                        labelB = ((AppWidgetProviderInfo) b).loadLabel(mPackageManager);
                    }
                } else {
    ...
    public void dumpState() {
        DebugLog.instance.outputLog(TAG, "mCallbacks=" + mCallbacks);
        ...
        if (mLoaderTask != null) {
            ((LoaderTask) mLoaderTask).dumpState();
        } else {
            DebugLog.instance.outputLog(TAG, "mLoaderTask=null");
        }
    }

    protected class PackageUpdatedTask implements Runnable {
        ...
        public void run() {
            ...
            switch (mOp) {
                case OP_ADD:
                    for (int i=0; i<N; i++) {
                        DebugLog.instance.outputLog(TAG, "mAllAppsList.addPackage " + packages[i]);
                        mBgAllAppsList.addPackage(context, packages[i]);
                    }
                    break;
                case OP_UPDATE:
                    for (int i=0; i<N; i++) {
                        DebugLog.instance.outputLog(TAG, "mAllAppsList.updatePackage " + packages[i]);
                ...
                case OP_UNAVAILABLE:
                    for (int i=0; i<N; i++) {
                        DebugLog.instance.outputLog(TAG, "mAllAppsList.removePackage " + packages[i]);
            ...
            final Callbacks callbacks = mCallbacks != null ? (Callbacks) mCallbacks.get() : null;
            if (callbacks == null) {
            ...
            if (added != null) {
                ...
                mHandler.post(new Runnable() {
                    public void run() {
                        Callbacks cb = mCallbacks != null ? (Callbacks) mCallbacks.get() : null;
    ...
    protected void bindWidgetsAndShortcus() {
     ...
     *   - all apps icons
     */
    protected class LoaderTask extends AbstractLoaderTask {
        ...
        protected boolean mIsLoadingAndBindingWorkspace;
        ...
        protected boolean isLoadingWorkspace() {
        ...
        protected void runBindSynchronousPage(int synchronousBindPage) {
        ...
        protected void waitForIdle() {
            ...
            // down.
            synchronized (LoaderTask.this) {
                final long workspaceWaitTime = DEBUG ? SystemClock.uptimeMillis() : 0;

                mHandler.postIdle(new Runnable() {
                        public void run() {
                            synchronized (LoaderTask.this) {
                                mLoadAndBindStepFinished = true;
                                DebugLog.instance.outputLog(TAG, "done with previous binding step");
                ...
                DebugLog.instance.outputLog(TAG, "waited "
        ...
        protected boolean checkItemPlacement(ItemInfo occupied[][][], ItemInfo item) {
            int containerIndex = item.screen;
            if (item.container == LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
                // Return early if we detect that an item is under the hotseat button
                if (mCallbacks == null || ((Callbacks) mCallbacks.get()).isAllAppsButtonRank(item.screen)) {
                    return false;
                }

                // We use the last index to refer to the hotseat and the screen as the rank, so
                // test and update the occupied state accordingly
                if (occupied[Workspace.MAX_SCREEN_COUNT][item.screen][0] != null) {
                    Log.e(TAG, "Error loading shortcut into hotseat " + item
                        + " into position (" + item.screen + ":" + item.cellX + "," + item.cellY
                        + ") occupied by " + occupied[Workspace.MAX_SCREEN_COUNT][item.screen][0]);
                    return false;
                } else {
                    occupied[Workspace.MAX_SCREEN_COUNT][item.screen][0] = item;
                    return true;
                }
        ...
        protected void filterCurrentDummyItems(int currentScreen,
                ArrayList<DummyInfo> dummyItems,
                ArrayList<DummyInfo> currentScreenDummyItems,
                ArrayList<DummyInfo> otherScreenDummyItems) {

            if (currentScreen < 0) {
                currentScreenDummyItems.addAll(dummyItems);
            }

            for (DummyInfo dummy : dummyItems) {
                if (dummy == null) continue;
                if (dummy.container == LauncherSettings.Favorites.CONTAINER_DESKTOP &&
                        dummy.screen == currentScreen) {
                    currentScreenDummyItems.add(dummy);
                } else {
                    otherScreenDummyItems.add(dummy);
                }
            }
        }
        ...
        private void sortWorkspaceItemsSpatially(ArrayList<ItemInfo> workspaceItems) {
            ...
            Collections.sort(workspaceItems, new Comparator<ItemInfo>() {
                ...
                public int compare(ItemInfo lhs, ItemInfo rhs) {
                    ...
                    int containerOffset = screenOffset * (5 + 1); // +1 hotseat
//                    int containerOffset = screenOffset * (Launcher.SCREEN_COUNT + 1); // +1 hotseat
        ...
        protected void bindWorkspaceItems(final Callbacks oldCallbacks,
                final ArrayList<ItemInfo> workspaceItems,
                final ArrayList<LauncherAppWidgetInfo> appWidgets,
                final ArrayList<DummyInfo> dummyItems,
            ...
            N = dummyItems.size();
            for (int i = 0; i < N; i++) {
                final DummyInfo dummy = dummyItems.get(i);
                final Runnable r = new Runnable() {
                    public void run() {
                        Callbacks callbacks = (Callbacks) tryGetCallbacks(oldCallbacks);
                        if (callbacks != null) {
                            callbacks.bindDummyItem(dummy);
                        }
                    }
                };
                if (postOnMainThread) {
        ...
        /**
         * 読み込んだWorkspaceのデータを実際のViewにバインドする。
         * (Viewを操作するのでメイン(UI)スレッドで行う)
        ...
        protected void loadAndBindAllApps() {
            if (!mAllAppsLoaded) {
                loadAllAppsByBatch();
                synchronized (this) {
                    if (mStopped) {
        ...
        protected void onlyBindAllApps() {
            final Callbacks oldCallbacks = (Callbacks) mCallbacks.get();
            if (oldCallbacks == null) {
            ...
            Runnable r = new Runnable() {
                public void run() {
                    final Callbacks callbacks = (Callbacks) tryGetCallbacks(oldCallbacks);
                    if (callbacks != null) {
        ...
        protected void loadAllAppsByBatch() {
        ...
        public void dumpState() {
            synchronized (sBgLock) {
                DebugLog.instance.outputLog(TAG, "mLoaderTask.mContext=" + mContext);
                DebugLog.instance.outputLog(TAG, "mLoaderTask.mIsLaunching=" + mIsLaunching);
                DebugLog.instance.outputLog(TAG, "mLoaderTask.mStopped=" + mStopped);
                DebugLog.instance.outputLog(TAG, "mLoaderTask.mLoadAndBindStepFinished=" + mLoadAndBindStepFinished);
                DebugLog.instance.outputLog(TAG, "mItems size=" + sBgWorkspaceItems.size());
            }
        }
    }
}
