...
public class WidgetPreviewLoader {
    ...
    private PagedViewGridLayout mWidgetSpacingLayout;
    ...
    public void setPreviewSize(int previewWidth, int previewHeight,
            PagedViewGridLayout widgetSpacingLayout) {
    ...
    public Bitmap generateWidgetPreview(AppWidgetProviderInfo info, int cellHSpan,
        ...
        cellHSpan = cellHSpan / 2;
        cellVSpan = cellVSpan / 2;

        Drawable drawable;
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            drawable = loadPreviewImage(info, 0);
        } else {
        ...
        if (widgetPreviewExists) {
            ...
            BitmapDrawable previewDrawable = (BitmapDrawable) ContextCompat.getDrawable(mContext, R.drawable.widget_preview_tile);
            ...
            try {
                ...
                if (info.icon > 0)
                    icon = mIconCache.getFullResIcon(info.provider.getPackageName(), info.icon);
                if (icon != null) {
    ...
    private Drawable loadPreviewImage(AppWidgetProviderInfo info, int density) {
        try {
            if (info.previewImage > 0) {
                ...
                android.content.pm.ApplicationInfo appInfo = mContext.getPackageManager().getApplicationInfo(packageName, 0);
                Resources resources = mContext.getPackageManager().getResourcesForApplication(appInfo);

//                Bitmap bmp = BitmapUtils.decodeToBitmap(resources, info.previewImage);
//                return new BitmapDrawable(resources, bmp);
                return resources.getDrawableForDensity(info.previewImage, density);
            }
