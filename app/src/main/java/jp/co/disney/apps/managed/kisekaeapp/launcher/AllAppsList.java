...
class AllAppsList {
    ...
    static final String TAG = "AllAppsList";
    ...
    public void updateKisekaeInfo(String themeId, boolean inAppTheme, int contentsType) {

        ArrayList<ApplicationInfoBase> tmpData = new ArrayList<ApplicationInfoBase>();
        for (ApplicationInfo item : data) {
            tmpData.add(item);
        }

        mIconCache.setCurrentKisekaeMap(KisekaeLauncherModel.createKisekaeMap(
                tmpData, mIconCache.getKisekaeMap(), themeId, inAppTheme, contentsType));
    }
