...
import jp.co.disney.apps.managed.kisekaeapp.system.file.LocalCacheFileHandleResolver;
    ...
    public class IconPickerScreen extends ApplicationAdapter{

        private IconPickerActivity myIconActivity;
...
//		final int MANAGER_MAX = 32;//架空スレッド数
    ...
    //タッチの状態
        ...
        public void setActivity(IconPickerActivity me){
            this.myIconActivity = me;
        }
...
//	        cto = ContentsOperatorForCatalog.op.getNewArrivalContents();
            cto = ContentsOperatorForIcon.op.getNewArrivalContents();
...
//				ContentsOperatorForCatalog.op.callChangeFavoriteTask(cto.get(num));
                ...
                ContentsOperatorForIcon.op.callChangeFavoriteTask(cto.get(num));
        ...
        public void onFinishedSetFavorite(long setID) {
            ...
            //より安全
            boolean isFavorite = ContentsOperatorForIcon.op.getContentsDataFromAssetId(setID).isFavorite;
            if(isFavorite){
                 myIconActivity.CallToast("お気に入りに保存しました");
            }
			else{
				myIconActivity.CallToast("お気に入りから削除しました");
			}
            nowSettingFavorite = false;
        }
...
//			mLastY=mEaseY;
        ...
        void SetTheme(ContentsDataDto setCto){
    	    ...
    	    if(!SPPUtility.checkNetwork(myIconActivity.getApplicationContext())){
    	    	//ここでダイアログ表示
        	...
        	//メソッド呼び出し//TODO 仮
        	ContentsOperatorForIcon.op.callDownloadSkinTask(setCto);
//        	 ContentsOperatorForWidget.op.callDownloadSkinTask(ContentsOperatorForWidget.op.getContentsDataFromAssetId(10000005193L));
        ...
        Intent iconIntent;
        public void onFinishedDownloadIconThumbsSeparate(boolean isList,Intent intent) {
    		DebugLog.instance.outputLog("catalog", "アイコンサムネイル取得成功");
            iconIntent = intent;
            if(isList){
                ContentsOperatorForIcon.op.callIconSelectListActivity(cto.get(nowIndex[state_setNum]));
            }
            else{
            	ContentsOperatorForIcon.op.callIconSelectGridActivity(cto.get(nowIndex[state_setNum]),iconIntent);
            }
        	resetStateBtn();
        	nowSettingThemes = false;
        	nowTouchState = TouchState.TOUCH_SCROLL;
