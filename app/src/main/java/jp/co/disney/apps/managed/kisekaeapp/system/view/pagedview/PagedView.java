...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    protected PageIndicatorView mPageIndicator = null;
    ...
    protected boolean canLoop() {
        return (mIsLooping && getPageCount() > 1);
    }

    public int getPageSpacing() {
        return (mPageSpacing == -1) ? 0 : mPageSpacing;
    }
    ...
    public void setCurrentPage(int currentPage) {
        ...
        currentPage = Math.min(currentPage, childCount - 1);

        // TODO:現在のページ数とmChildOffsets配列の長さが合わないことがある。
        // PageViewはもともとページ数固定でつくられているため、
        // ページ数が動的に変化する場合の考慮を見直す必要がありそう。
        if (mChildOffsets == null || childCount != mChildOffsets.length) {
        ...
        if (mPageIndicator != null) {
            mPageIndicator.updateCurrentPage(currentPage);
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        DebugLog.instance.outputLog(TAG, "PagedView.onMeasure(): " + widthSize + ", " + heightSize);
        final int childCount = getChildCount();
        for (int i = 0; i < childCount; i++) {
            ...
            DebugLog.instance.outputLog(TAG, "\tmeasure-child" + i + ": " + child.getMeasuredWidth() + ", "
        ...
        if (childCount > 0) {
            DebugLog.instance.outputLog(TAG, "getRelativeChildOffset(): " + getMeasuredWidth() + ", "
                    + getChildWidth(0));
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        DebugLog.instance.outputLog(TAG, "PagedView.onLayout()");
        final int verticalPadding = getPaddingTop() + getPaddingBottom();
        final int childCount = getChildCount();

        final int startIndex = 0;
        final int endIndex = childCount;
        final int delta = 1;
        ...
        for (int i = startIndex; i != endIndex; i += delta) {
            ...
            if (child.getVisibility() != View.GONE) {
                ...
                DebugLog.instance.outputLog(TAG, "\tlayout-child" + i + ": " + childLeft + ", " + childTop);
    ...
    protected void acceleratedOverScroll(float amount) {
        ...
        final boolean canLoop = canLoop();

        // We want to reach the max over scroll effect when the user has
        // over scrolled half the size of the screen
        float f;
        if (canLoop) {
            // ループさせる場合はスクロール量の補正をしない
            f =  amount / screenSize;
        } else {
            f = OVERSCROLL_ACCELERATE_FACTOR * (amount / screenSize);
        }
        ...
        if (amount < 0) {
            ...
            if (canLoop && overScrollAmount == -screenSize
                    && mScroller.isFinished()) {
                // タッチ状態を維持したままループさせるための対処
                final int endPage = getChildCount() - 1;
                final int newOverScrollX = mMaxScrollX + mPageSpacing
                        - getMeasuredWidth()
                        + getPageAt(endPage).getMeasuredWidth();
                if (mOverScrollX != newOverScrollX) {
                    mOverScrollX = newOverScrollX;
                    mUnboundedScrollX = mOverScrollX;
                    mCurrentPage = endPage;
                    mNextPage = INVALID_PAGE;
                    super.scrollTo(mMaxScrollX, getScrollY());
                    mScroller.setFinalX(mMaxScrollX);
                    mScroller.forceFinished(true);
                }
            ...
            if (canLoop && overScrollAmount == screenSize
                    && mScroller.isFinished()) {
                // タッチ状態を維持したままループさせるための対処
                int newOverScrollX = -mPageSpacing + getMeasuredWidth()
                        - getPageAt(0).getMeasuredWidth();
                if (mOverScrollX != newOverScrollX) {
                    mOverScrollX = newOverScrollX;
                    mUnboundedScrollX = mOverScrollX;
                    mCurrentPage = 0;
            ...
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (Math.abs(deltaX) >= 1.0f) {
                    ...
                    if (!mDeferScrollUpdate) {
                        ...
                        DebugLog.instance.outputLog(TAG, "onTouchEvent().Scrolling: " + deltaX);
                    } else {
                        invalidate();
                    }
                    mLastMotionX = x;
            ...
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
                    ...
                    if (mCurrentPage > 0) {
                        ...
                        if (canLoop()) {
                            // ループ時に最初のページから最後のページに遷移させるための対処
                            final int endPage = getChildCount() - 1;
                            mOverScrollX = mMaxScrollX + mPageSpacing
                                    + getPageAt(endPage).getMeasuredWidth()
                                    + mOverScrollX;
                            mUnboundedScrollX = mOverScrollX;
                            super.scrollTo(mMaxScrollX, getScrollY());
                            ...
                            mCurrentPage = endPage;
                            mNextPage = INVALID_PAGE;
                            snapToPageWithVelocity(endPage, velocityX);
                    ...
                    if (mCurrentPage < getChildCount() - 1) {
                        ...
                        if (canLoop()) {
                            // ループ時に最後のページから最初のページに遷移させるための対処
                            mOverScrollX = -mMaxScrollX - getMeasuredWidth()
                                    + mOverScrollX;
                            mUnboundedScrollX = mOverScrollX;
                            super.scrollTo(0, getScrollY());
                            mScroller.setFinalX(0);
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_SCROLL: {
                    ...
                    if (hscroll != 0 || vscroll != 0) {
                        boolean isForwardScroll = (hscroll > 0 || vscroll > 0);
                        if (isForwardScroll) {
                            scrollRight();
                        } else {
                            scrollLeft();
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        if (mChildOffsets == null || whichPage >= mChildOffsets.length) return;

//        DebugLog.instance.outputLog(TAG, "snapToPage.getChildOffset(): " + getChildOffset(whichPage));
//        DebugLog.instance.outputLog(TAG, "snapToPageWithVelocity.getRelativeChildOffset(): "
//                + getMeasuredWidth() + ", " + getChildWidth(whichPage));
    ...
    public void snapToPage(int whichPage, int duration) {
        ...
        DebugLog.instance.outputLog(TAG, "snapToPage.getChildOffset(): " + getChildOffset(whichPage));
        DebugLog.instance.outputLog(TAG, "snapToPage.getRelativeChildOffset(): " + getMeasuredWidth() + ", "
    ...
    public void snapToPage(int whichPage, int delta, int duration) {
        ...
        if (mPageIndicator != null) {
            mPageIndicator.updateCurrentPage(whichPage);
        }
    ...
    public void loadAssociatedPages(int page, boolean immediateAndOnly) {
        if (mContentIsRefreshable) {
            ...
            if (page < count) {
                ...
                DebugLog.instance.outputLog(TAG, "loadAssociatedPages: " + lowerPageBound + "/"
                        + upperPageBound);
...
//                mScrollIndicator = (View) (parent.findViewById(R.id.paged_view_indicator));
    ...
    public void showScrollingIndicator(boolean immediately) {
        ...
        if (mScrollIndicator != null) {
            ...
            if (immediately || mScrollingPaused) {
                ...
                mScrollIndicatorAnimator = ViewAnimUtils.ofFloat(mScrollIndicator, "alpha", 1f);
    ...
    public void hideScrollingIndicator(boolean immediately) {
        ...
        if (mScrollIndicator != null) {
            ...
            if (immediately || mScrollingPaused) {
                ...
                mScrollIndicatorAnimator = ViewAnimUtils.ofFloat(mScrollIndicator, "alpha", 0f);
    ...
    public void setPageIndicator(PageIndicatorView pageIndicator) {
