...
import jp.co.disney.apps.managed.kisekaeapp.system.view.animation.InterruptibleInOutAnimator;
...
public abstract class CellLayout extends ViewGroup {
    ...
    protected ArrayList<FolderIconAnimator> mAnimFolderIcons = new ArrayList<FolderIconAnimator>();
    ...
    protected static final int BASE_CELL_WIDTH = 135;
    protected static final int BASE_CELL_WIDTH_HOTSEAT = 108;
    protected static final int BASE_CELL_HEIGHT = 138;
    protected static final int BASE_CELL_HEIGHT_HOTSEAT = 108;
    protected static final int BASE_ICON_SIZE = 144;
    protected static final int BASE_FOLDER_ICON_SIZE = 180;

    protected int mIconSize;
    protected int mCellPadTop;
    protected int mTextPad;
    protected int mFolderIconSize;
    protected int mFolderCellPadTop;
    ...
    public int getFolderCellPaddingTop() {
        return mFolderCellPadTop;
    }
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mCountX = a.getInt(R.styleable.CellLayout_cellCountX, 4);
        mCountY = a.getInt(R.styleable.CellLayout_cellCountY, 5);
        a.recycle();
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    if (outline == null) {
                        ...
                        if (debug) {
                            ...
                            DebugLog.instance.outputLog(TAG, "anim " + thisIndex + " update: " + val +
    ...
    protected void invalidateBubbleTextView(BubbleTextView icon) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // フォルダアニメーション
        for (int i = 0; i < mAnimFolderIcons.size(); i++) {
            FolderIconAnimator animFolderIcon = mAnimFolderIcons.get(i);

            Drawable d = FolderIconAnimator.sAnimFolderIconDrawable;
            int width = (int) animFolderIcon.getAnimFolderIconSize();
            int height = width;
            cellToPoint(animFolderIcon.mCellX, animFolderIcon.mCellY, mTempLocation);

            int centerX = mTempLocation[0] + mCellWidth * 2 / 2;
            int centerY = mTempLocation[1] + mCellPadTop + mIconSize / 2;
        ...
        // フォルダを開いた時の元のフォルダ(ドックで確認可能)
        if (mFolderLeaveBehindCell[0] >= 0 && mFolderLeaveBehindCell[1] >= 0) {

            Drawable d = FolderIconAnimator.sAnimFolderIconDrawable;
            int width = FolderIconAnimator.sBaseFolderIconSize;
            int height = width;
    ...
    public void showFolderAccept(FolderIconAnimator fra) {
        mAnimFolderIcons.add(fra);
    }

    public void hideFolderAccept(FolderIconAnimator fra) {
        if (mAnimFolderIcons.contains(fra)) {
            mAnimFolderIcons.remove(fra);
        }
        invalidate();
    }
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        if (child instanceof BubbleTextView) {
            ...
            bubbleChild.setTextColor(ContextCompat.getColor(getContext(), R.color.workspace_icon_text_color));

        } else if (child instanceof FolderIcon) {
            FolderIcon folderIcon = (FolderIcon) child;
            folderIcon.setFolderLayout(mFolderIconSize , mFolderCellPadTop, mScale, mCellWidth * 2, mCellPadTop);
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthSpecMode == MeasureSpec.AT_MOST) {
            newWidth = getPaddingLeft() + getPaddingRight() + (int) Math.ceil(mCountX * mfCellWidth);
            newHeight = getPaddingTop() + getPaddingBottom() + (int) Math.ceil(mCountY * mfCellHeight);
    ...
    void visualizeDropLocation(View v, Bitmap dragOutline, int originX, int originY, int cellX,
        ...
        if (cellX != oldDragCellX || cellY != oldDragCellY) {
            ...
            if (v != null && dragOffset == null) {
                ...
                left += ((int) Math.floor(mfCellWidth * spanX) - dragOutline.getWidth()) / 2;
            } else {
                if (dragOffset != null && dragRegion != null) {
                    // Center the drag region *horizontally* in the cell and apply a drag
                    // outline offset
                    left += dragOffset.x + ((int) Math.floor(mfCellWidth * spanX) - dragRegion.width()) / 2;
                    top += dragOffset.y;
                } else {
                    // Center the drag outline in the cell
                    left += ((int) Math.floor(mfCellWidth * spanX) - dragOutline.getWidth()) / 2;
                    top += ((int) Math.floor(mfCellHeight * spanY) - dragOutline.getHeight()) / 2;
                }
            }
    ...
    */
    ...
    public void cellToRect(int cellX, int cellY, int cellHSpan, int cellVSpan, Rect resultRect) {
        final float cellWidth = mfCellWidth;
        final float cellHeight = mfCellHeight;

        final int hStartPadding = getPaddingLeft();
        final int vStartPadding = getPaddingTop();

        float width = cellHSpan * cellWidth;
        float height = cellVSpan * cellHeight;

        int x = hStartPadding + (int) Math.ceil(cellX * cellWidth);
        int y = vStartPadding + (int) Math.ceil(cellY * cellHeight);

        resultRect.set(x, y, x + (int) Math.floor(width), y + (int) Math.floor(height));
    }
    ...
    public static int[] rectToCell(Resources resources, int width, int height, int[] result) {
        ...
        int dw = resources.getDisplayMetrics().widthPixels;

        int actualWidth = (int) Math.floor(BASE_CELL_WIDTH * dw / (float) 1080);
        int actualHeight = (int) Math.floor(BASE_CELL_HEIGHT * dw / (float) 1080);
    ...
    public int[] cellSpansToSize(int hSpans, int vSpans) {
        int[] size = new int[2];
        size[0] = (int) Math.ceil(hSpans * mfCellWidth);
        size[1] = (int) Math.ceil(vSpans * mfCellHeight);
        return size;
    }
     ...
     */
    public void calculateSpans(ItemInfo info, int iconSpan) {
        final int minWidth;
        final int minHeight;
        ...
        if (info instanceof LauncherAppWidgetInfo) {
            ...
            // ウィジェットでない
            info.spanX = info.spanY = iconSpan;
            return;
        }
    ...
    static boolean findVacantCell(int[] vacant, int spanX, int spanY,
        ...
        for (int y = 0; y < yCount; y++) {
            for (int x = 0; x < xCount; x++) {
                if (!occupied[x][y] && x + spanX <= xCount && y + spanY <= yCount) {
...
out:                for (int i = x; i < x + spanX; i++) {
    ...
    public int getDesiredWidth() {
        return getPaddingLeft() + getPaddingRight() + (int) Math.ceil(mCountX * mfCellWidth);
    }

    public int getDesiredHeight()  {
        return getPaddingTop() + getPaddingBottom() + (int) Math.ceil(mCountY * mfCellHeight);
    }
    ...
    public static class LayoutParams extends ViewGroup.MarginLayoutParams {
        ...
        public void setup(float cellWidth, float cellHeight, int colCount) {
            if (isLockedToGrid) {
